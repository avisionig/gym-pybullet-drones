...
#     # Train the model
#     model.learn(total_timesteps=int(5000),  # Set training duration
...
DEFAULT_OUTPUT_FOLDER = 'spaceship_results'
DEFAULT_OBS = ObservationType('kin')  # 'kin' or 'rgb'
DEFAULT_ACT = ActionType('rpm')  # Keep this as 'rpm'
...
def run(output_folder=DEFAULT_OUTPUT_FOLDER, gui=DEFAULT_GUI, plot=True, record_video=DEFAULT_RECORD_VIDEO, local=True):
    ...
    # Train the model
    model.learn(
        total_timesteps=int(5000),  # Set training duration
        callback=eval_callback,
        log_interval=1000
    )
    ...
    for i in range((test_env.EPISODE_LEN_SEC+2)*test_env.CTRL_FREQ):
        ...
        # Ensure action is flattened
        action_flat = action.flatten()
        print(f"[DEBUG] Predicted action: {action_flat}")  # Debug the action shape
